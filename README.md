...
The canvas fills the element and sits behind its children, so headings, buttons and cards go on top as usual. Facets are lit from one side, which makes the highlights and shadows cluster the way they do on a real crumpled surface instead of reading as random noise. A light film grain takes the flat digital edge off.
...
| `cell`     | `170`                      | Approximate facet size in CSS pixels. Smaller cells mean busier surfaces.        |
| `jitter`   | `0.6`                      | How irregular the facets are, `0` (a neat grid) to `1`.                          |
| `depth`    | `0.5`                      | Contrast between lit and shaded facets, `0` (flat) to `1`.                       |
